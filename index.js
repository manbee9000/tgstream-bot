...
async function exchangeCodeForToken(code) {
  ...
  daTokenExpiresAt = new Date(
    Date.now() + (data.expires_in ? data.exires_in * 1000 : 3600 * 1000)
  );
...
async function ensureDaAccessToken() {
  ...
  try {
    ...
    daTokenExpiresAt = new Date(
      Date.now() + (data.exires_in ? data.exires_in * 1000 : 3600 * 1000)
    );
...
bot.on("message", async (msg) => {
  try {
    ...
    if (text === "💸 Подключить донат к стриму") {
      await bot.sendMessage(
        chatId,
        "Чтобы подключить донат к стриму, используйте команду:\n\n" +
          "/donate ИМЯ_СТРИМЕРА_НА_DA\n\n" +
          "Например:\n" +
          "/donate mycoolstreamer\n\n" +
          "После этого в посте со стримом у зрителей появится кнопка «💸 Донат».",
        { reply_markup: buildMainMenu() }
      );
      return;
    }
    ...
    if (text === "📘 Инструкция") {
      await bot.sendMessage(
        ...
        "📘 *Краткая инструкция по боту:*\n\n" +
          ...
          "• 💸 Подключить донат — команда /donate ИМЯ_СТРИМЕРА_НА_DA\n" +
